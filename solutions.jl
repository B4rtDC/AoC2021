...
# ╔═╡ 89006086-bff2-4d3f-a4c7-5e250d4e64af
let
	input = readlines("./D13-2.txt")
	
	function inputparser(input)
		r = r"fold along (x|y)=(\d+)"
		points = Vector{Tuple{Int,Int}}()
		folds = Vector{Tuple{Symbol,Int}}()
		for line in input
			if occursin(",", line)
				push!(points, Tuple(parse.(Int, split(line,",")) .+ 1))
			elseif occursin("fold", line)
				c = match(r,line).captures
				push!(folds, (Symbol(c[1]), parse(Int, c[2])+1))
			end
		end

		m = zeros(Bool, maximum(x->x[1], points),maximum(x->x[2], points))
		for p in points
			m[p[1], p[2]] = true
		end
		
		return permutedims(m), folds
	end

	
	p,f = inputparser(input)
	...
	for fold in f
		dir, loc = fold
		if dir == :y
			uppart = reverse(res[loc+1:end,:], dims=1)
			(n,m) = size(uppart)
			res[loc-n:loc-1, :] = res[loc-n:loc-1, :] .|| uppart
			res = res[1:loc-1,:]
		else
			left = reverse(res[:, loc+1:end], dims=2)
			(n,m) = size(left)
			res[:, loc-m:loc-1] = res[:, loc-m:loc-1] .|| left
			res = res[:,1:loc-1]
		end
		push!(dotcount, sum(res))
	end
	end
	...
	using Plots
	res_2 = heatmap(res, yflip=true, size=(300,100), legend=false, axis=false,ticks=false)
...
let
	input = readlines("./D12-4.txt")
	
	function netbuilder(input)
		net = Dict{String, Vector{String}}()
		for line in input
			s, d = split(line,"-")
			push!(get!(net, s, String[]), d)
			push!(get!(net, d, String[]), s)
		end
		multipass = Dict(map(x->x => isuppercase(x[1]), collect(keys(net))))
		
		return net, multipass
	end

	function crawler(net::Dict{String,Vector{String}}, # net layout
					 multipass::Dict{String, Bool}, # multiple passes allowed
					 out::Vector{Vector{String}}=Vector{Vector{String}}(), # output
					 mypath::Vector{String}=String[], # current path
				     visited::Dict{String,Int}=Dict(key => 0 for key in keys(net)), # visit count
					 current::String="start"; # current node;
					 neighfun::Function=neigh_1
					)
		# update path
		push!(mypath, current)
		# update counts
		...
		# identify possible neighbors
		neighbors = neighfun(net,multipass, visited, current, mypath)
		if length(neighbors) == 0 
			return
		else
			for n in neighbors
				crawler(net, multipass,out, copy(mypath), copy(visited), n,neighfun=neighfun)
			end
		end
	...
	function neigh_1(net, multipass, visited, current, path; maxvisit::Int=1)
		return filter(x -> multipass[x] ? true : visited[x] < maxvisit, net[current])
	end

	function neigh_2(net, multipass, visited, current, path)
		allowed = String[]
		lowernodes = [x for x in path[2:end] if islowercase(x[1])]
		smalltwice = length(lowernodes) > length(unique(lowernodes))
		for node in net[current]
	...
	net, multipass = netbuilder(input)

 	res_1 = length(crawler(net, multipass))
	res_2 = length(crawler(net, multipass, neighfun=neigh_2))

	(res_1, res_2)
	
	end

# ╔═╡ 940a6b69-dd3f-4fda-9705-6699ca640981
md"""
## Day 13
"""

# ╔═╡ 1fa3996e-2e2b-4e0e-a572-e64124537e14
md"""
## Day 14

"""

# ╔═╡ e3dff749-ecc7-4bf5-b477-b1b039880971
let
	# part 1 (slow method)
	input = readlines("./D14-2.txt")
	start = input[1]	
	rules = Dict(v[1] => v[2] for v in split.(input[3:end], " -> "))
	N = 40
	#=
	buffer = IOBuffer()
	function polymerize(molecule::String, rules::Dict; buffer=IOBuffer())
		write(buffer, molecule[1])
		for i in 1:length(molecule)-1

			write(buffer, rules[@view molecule[i:i+1]])
			write(buffer, molecule[i+1])
		end
	...
	res = start
	for _ in 1:N
		res = polymerize(res, rules;buffer=buffer)
	end
	=#
	
	# part 2 (fast method, cf. exponential fishes)
	newstart = Dict{String, Int}()
	for i in 1:length(start)-1
		newstart[start[i:i+1]] = get!(newstart, start[i:i+1], 0) + 1
	end

	"""
		fastpolymerize(molecule, rules)

	store molecule as dictionary of pairs
	"""
	function fastpolymerize(molecule, rules)
		new = Dict{String, Int}()
		for (pair, val) in molecule
			created = ("$(pair[1])$(rules[pair][1])", "$(rules[pair][1])$(pair[2])")
			for mol in created
				new[mol] = get!(new, mol, 0) + val
			end
		end
		return new
	end
	...
	"""
		getscores(start, molecule)

	obtain scores from molecule as dict, each char is counted twice, except for the one at the start & the end
	"""
	function getscores(start::String, molecule::Dict)
		res = Dict{Char, Int}()
		res[start[1]] = 1
		res[start[end]] = 1
		for (pair, val) in molecule
			for char in pair
				res[char] = get!(res, char, 0) + val
			end
		end
		return (maximum(values(res)) - minimum(values(res)))÷2
	end

	res_1 = 0
	res_2 = 0
	for it in 1:N
		newstart = fastpolymerize(newstart, rules)
		if it == 10
			res_1 = getscores(start, newstart)
		elseif it == 40
			res_2 = getscores(start, newstart)
		end
	end
...
# ╔═╡ 06343268-7390-4c03-9c46-a3e40b93715a
md"""
## Day 15
"""

# ╔═╡ 267d1d75-f25b-4def-81ad-37190b036610
let
	input = readlines("./D15-1.txt")
end
